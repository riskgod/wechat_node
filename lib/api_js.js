...
Ticket.prototype.isValid =  () =>{
  return !!this.ticket && (new Date().getTime()) < this.expireTime;
};


exports.registerTicketHandle =  (getTicketToken, saveTicketToken) =>{
...
exports.getTicket =  (type, callback)=> {
  this.preRequest(this._getTicket, arguments);
};

exports._getTicket =  (type, callback) =>{
  if (typeof type === '') {
    callback = type;
    type = 'jsapi';
  }
...
let sign =  (nonceStr, jsapi_ticket, timestamp, url)=> {
  let ret = {
...
let signCardExt = (api_ticket, card_id, timestamp, code, openid, balance)=> {
...
let preRequestJSApi =  (method, args, retryed) =>{
  let that = this;
 ...
 */
let preRequestWxCardApi = (method, args, retryed)=> {
  let that = this;
  let callback = args[args.length - 1];
...
exports.getJsConfig =  (param, callback)=> {
  preRequestJSApi.call(this, this._getJsConfig, arguments);
};
exports._getJsConfig =  (param, callback) =>{
  let that = this;
...
exports.getCardExt =  (param, callback) =>{
  preRequestWxCardApi.call(this, this._getCardExt, arguments);
};

exports._getCardExt =  (param, callback)=> {
...
exports.getLatestTicket =  (callback) =>{
  preRequestJSApi.call(this, this._getLatestTicket, arguments);
};
exports._getLatestTicket =  (callback) =>{
